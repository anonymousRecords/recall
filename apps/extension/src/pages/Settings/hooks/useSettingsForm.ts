...
export function useSettingsForm() {
	...
	const [intervals, setIntervals] = useState(settings.reviewIntervals.join(", "));
	...
	const validate = (): string | null => {
		...
		if (!parsed) return "복습 주기는 양수로 입력해주세요 (예: 1, 3, 7, 14, 30)";
		return null;
	};
	...
	const save = async () => {
		const parsed = parseIntervals(intervals);
		if (!parsed) return;
		await saveSettings({ reviewIntervals: parsed });
	};
	...
	return { intervals, setIntervals, validate, save, reset };
}
