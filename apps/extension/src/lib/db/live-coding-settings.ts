...
export async function getLiveCodingSettings(): Promise<LiveCodingSettings> {
	const [settings, apiKey] = await Promise.all([
		...
		getStorageItem(API_KEY_STORAGE_KEY, ""),
	]);
...
export async function updateLiveCodingSettings(
	...
	if (updates.apiKey !== undefined) {
		await setStorageItem(API_KEY_STORAGE_KEY, updates.apiKey);
	}
	...
	return {
		...newDbSettings,
		apiKey: updates.apiKey ?? (await getStorageItem(API_KEY_STORAGE_KEY, "")),
	};
}
